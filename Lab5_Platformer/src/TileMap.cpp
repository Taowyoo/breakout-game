...
TileMap::TileMap(std::string tileSheetFileName, int rows, int cols,
                 int _TileWidth, int _TileHeight, int _mapX, int _mapY,
                 int _cam_x, int _cam_y, SDL_Renderer* ren) {
  if (nullptr == ren) {
...
void TileMap::SetCameraPosition(int x, int y) {
  cam_x = x;
  cam_y = y;
}

// Returns what the tile is at a specific position.
int TileMap::GetTileType(int x, int y) { return m_Tiles[y * m_MapX + x]; }
  ...
  for (int y = 0; y < m_MapY; y++) {
    for (int x = 0; x < m_MapX; x++) {
      ...
      if (currentTile > -1) {
        ...
        Dest.x = x * m_TileWidth - cam_x;
        Dest.y = y * m_TileHeight - cam_y;
        Dest.w = m_TileWidth;
        Dest.h = m_TileHeight;
