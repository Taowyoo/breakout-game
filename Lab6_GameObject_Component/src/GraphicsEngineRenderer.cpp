...
GraphicsEngineRenderer::GraphicsEngineRenderer(int w, int h)
  ...
  if (SDL_Init(SDL_INIT_EVERYTHING) < 0) {
    ...
    // Create window
    m_window = SDL_CreateWindow("Lab", 100, 100, m_screenWidth, m_screenHeight,
                                SDL_WINDOW_SHOWN);
...
void GraphicsEngineRenderer::RenderClear() { SDL_RenderClear(m_renderer); }

void GraphicsEngineRenderer::RenderPresent() { SDL_RenderPresent(m_renderer); }

// Get Pointer to Window
SDL_Window* GraphicsEngineRenderer::GetWindow() { return m_window; }

// Get Pointer to Renderer
SDL_Renderer* GraphicsEngineRenderer::GetRenderer() { return m_renderer; }
